$(function () {
    ...
    fetchBookings(null, null, null,  1, []);
    ...
    $(".filter").on('change', function () {
        fetchBookings(searchQuery.val(), from.val(), to.val(), 1, []);
    });
    ...
    $(".sortable").on('click', function () {
        sortOn($(this));
        fetchBookings(searchQuery.val(), from.val(), to.val(), 1, sort);
...
let page = 1;
let sort = JSON.parse(localStorage.getItem("pendingbookingsSorting")) || [];
...
function sortOn(selector) {
    ...
    localStorage.setItem("pendingbookingsSorting", JSON.stringify(sort));
}
function createRow(booking) {
    ...
    row.append($("<td></td>").text(booking.reservee.name));
    row.append($("<td></td>").text(booking.reservee.email));
    row.append($("<td></td>").text((booking.remarks ? booking.remarks : ' ')));
    row.append($("<td></td>").text(booking.status));

    row.append($("<td></td>").html(
        '<button type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Accept booking" onclick="onAcceptClick(' + booking.id + ')" class="btn btn-primary table-buttons"><i class="bi bi-check-lg"></i></button>' +
        '<button type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Reject booking" onclick="onRejectClick(' + booking.id + ')" class="btn btn-primary table-buttons"><i class="bi bi-x-lg"></i></button>' +
...
function fetchBookings(query, from, to, page, sort) {
        ...
        $('#demo').pagination({
            ...
            callback: function(data, pagination) {
                if (pagination.pageNumber !== page) {
                    ...
                    fetchBookings(query, from, to, pagination.pageNumber, sort);
                }
            }
        });
...
function onAcceptClick(id) {
    const onsuccess = function () {
        fetchBookings($("#searchQuery").val(), $("#fromPicker").val(), $("#toPicker").val(), page, sort);
        addSuccessToast("You successfully accepted a booking!");
    }
    fetch('POST', '/api/reservation/status/' + id + '/APPROVED', null, onsuccess, null);
}

function onRejectClick(id) {
    const onsuccess = function () {
        fetchBookings($("#fromPicker").val(), $("#toPicker").val(), page, sort);
        addSuccessToast("You successfully rejected a booking!");
    }
    fetch('POST', '/api/reservation/status/' + id + '/REJECTED', null, onsuccess, null);
}
        ...
        function onsuccess(devices) {
            ...
            devices.forEach(function (device) {
                if (device && device.position) {
                    const tooltipTitle = "Code: " + device.code;
